...
- Confirm `.env.local` and Vercel intentionally use the same Neon database before running local database commands.
...
- Review every pending migration, then apply committed migrations with `npm run db:migrate:deploy`.
- Never reset or seed the shared database unless deleting live data is intentional.
...
- Run `npm run frames:push` to upload approved frames to the database and R2 target from `.env.local`.
